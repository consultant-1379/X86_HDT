...
$(document).ready(function(){
        ...
        $("input[title='products_']").change(function(){
       ...
       $($("input[title='products_']")).each(function(){
...
function getVersionAjax(product_weight){
    ...
    var version='0';
     ...
     $.ajax({
            type: 'GET',
            async: false,
            url: 'clientAjax/get_versions_ajax.htm?product_weight=' + product_weight,
            timeout: 5000,
...
function getNetworkAjax(product_weight,version){
    ...
    $.ajax({
            type: 'GET',
            async: false,
            url: 'clientAjax/get_network_ajax.htm?product_weight=' + product_weight + '&version='+version,
            timeout: 5000,
            ...
            success: function(result_data) {    
                ...
                if($("input[title='networks_']").length ===1){
                    
                    network_weight=$("input[title='networks_']").val();
                    $("input[title='networks_']").attr("checked", "checked");
                     getBundlesAjax(product_weight,version,network_weight);
                }
                
                $("input[title='networks_']").change(function(){
                    var network_weight=0;
                     disableSaveRetreiveParameterLinks();
                 ...
                 $("#selected_action_div").html("");
                    ...
                    $($("input[title='networks_']")).each(function(){
...
function getBundlesAjax(product,version,network){
    ...
    //$("#loadingStatus").modal('show');
     ...
     $.ajax({
            type: 'GET',
            async: false,
            url: 'clientAjax/get_bundle_ajax.htm?product_weight='+ product + '&version=' + version +  '&network_weight=' + network ,
            timeout: 5000,
...
function getProductReleaseHelpMenuLinkAjax(product,version,network,bundle){
         ...
         $.ajax({
            type: 'GET',
            async: false,
            url: 'link/productReleaseHelpMenuLink.htm?product_weight='+ product + '&version=' + version +  '&network=' + network  + '&bundle=' + bundle,
            timeout: 5000,
...
function getProductReleaseParameters(product,version,network,bundle){
        ...
        var count=0;
         $.ajax({
            type: 'GET',
            async: false,
            url: 'clientAjax/preRequisites.htm?product_weight='+ product + '&version=' + version +  '&network=' + network  + '&bundle=' + bundle,
            timeout: 5000,
 ...
 function executeDimensioningParameterValidationAjax(post_data_json){
     ...
     $.ajax({
                        type: 'post',
                        async: false,
                        url: 'welcome/validateParameters.htm',
   ...
   parameter_list['site'] = site;
    parameter_list['role'] = role;
     ...
     $.ajax({
                        type: 'POST',
                        async: true,
                        url: 'dimensioner/roleHardwareAlternative.htm',
                        data:post_data_json,
 ...
 function downloadExcelReport(){
       ...
       var urlParameters = "";
            ...
            if(count>0){
                ...
                $("#excelFrame").attr('src','dimensioner/downloadExcelReport.htm'+ urlParameters);
            }
...
function recalculateParameters(role,site){
    ...
    if(parameter_list['applyToAllRoles']=='checked'){
        ...
        address = 'dimensioner/recalculateAllRole.htm';
        div_name_to_update = "ALL_ROLE_RESULT";
    }
    else {
        
       address= 'dimensioner/calculateIndividualRole.htm'
    ...
    for(i=0;i<elements.length;i++){
        if(elements[i].checked){
            ...
            $.ajax({
                        type: 'POST',
                        async: true,
                        url: 'hw_bundle/hardwareAlternative.htm?hardwareID='
                            + hardwareID + '&product_weight='+ product + '&version=' + version +  '&network=' + network  
                            + '&bundle=' + bundle + '&site=' + site + '&role=' + role,
                                  
                        beforeSend: function() {
                            
                            //$("#HardwareAlternativeModal_Role"+role + "Site"+site).modal('hide');
                            $("#loadingStatus").modal('show');
...
function deleteUserParameterSet(timeStamp){
    ...
    $.ajax({
                        type: 'POST',
                        async: false,
                        url: 'validation/deleteUserSaveParameters.htm',
                        data:post_data_json,
                        contentType: "application/json; charset=utf-8",
                        beforeSend: function() {
...
function updateAPPNumberQTY(app){
    ...
    $.ajax({
                        type: 'POST',
                        async: false,
                        url: 'dimensioner/updateHardwareAppQty.htm',
                        data:post_data_json,
                        contentType: "application/json; charset=utf-8",
                        ...
                        success: function(result_data) {    
                            ...
                            $("#APPNumber_QTY_Update").modal('show');
...
function getUserUniqueParameters(timeStamp){
     ...
     $.ajax({
                        type: 'POST',
                        async: false,
                        url: 'validation/getUserUniqueSaveParameters.htm',
                        data:post_data_json,
                        contentType: "application/json; charset=utf-8",
                        beforeSend: function() {
...
function retrieveUserSavedParameter(){
    ...
    $.ajax({
                        type: 'POST',
                        async: false,
                        url: 'validation/getUserSaveParameters.htm',
                        data:post_data_json,
                        contentType: "application/json; charset=utf-8",
                        beforeSend: function() {
...
function saveUserParameters(){
                ...
                $.ajax({
                        type: 'POST',
                        async: false,
                        url: 'validation/saveUserParameters.htm',
                        data:post_data_json,
                        contentType: "application/json; charset=utf-8",
   ...
   parameter_list['hardwareGen'] = element.value;
    ...
    parameter_list['product']= document.getElementById("product_weight").value;
     ...
     $.ajax({
            type: 'POST',
            async: true,
            url: 'dimensioner/updateHardwareToSelectedGeneration.htm',
            data:post_data_json,
            contentType: "application/json; charset=utf-8",
